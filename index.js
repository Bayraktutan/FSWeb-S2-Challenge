...
function cumlelereDonustur(someArray, string = ",") {
  let newArray = someArray.map((item) => item.join(string));
  return newArray;
}
...
function paragrafOlustur(someArray, firstCallBack, secondCallback) {
  let newArray = secondCallback(someArray, " ");
...
function emojileriDonustur(message, obj) {
  for (let i = 0; i < Object.keys(obj).length; i++) {
    message = message.replaceAll(
      Object.keys(obj)[i].toUpperCase(),
      Object.values(obj)[i]
    );
    message = message.replaceAll(
      Object.keys(obj)[i].toLowerCase(),
      Object.values(obj)[i]
    );
  }
  return message;
